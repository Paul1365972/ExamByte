...
class LandingPageControllerTest {
	...
	void test_landingPageLoggedIn() throws Exception {
		...
		// Act & Assert
		mvc.perform(get("/").with(SecurityMockMvcRequestPostProcessors.oauth2Login().attributes(attrs -> {
			attrs.put("id", 12345);
			attrs.put("login", "testuser");
			attrs.put("usertype", UserType.STUDENT);
		}))).andExpect(status().isOk()).andExpect(view().name("index")).andExpect(model().attribute("isLoggedIn", true))
