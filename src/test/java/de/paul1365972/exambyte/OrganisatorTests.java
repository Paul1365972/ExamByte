...
class OrganisatorTests {
    ...
    void test_listTests() throws Exception {
        ...
        de.paul1365972.exambyte.domain.Test test1 = new de.paul1365972.exambyte.domain.Test("Test 1", now, now.plusHours(2), now.plusHours(3));
        test1.setId(1L);
        de.paul1365972.exambyte.domain.Test test2 = new de.paul1365972.exambyte.domain.Test("Test 2", now.plusDays(1), now.plusDays(1).plusHours(2), now.plusDays(1).plusHours(3));
        test2.setId(2L);
        List<de.paul1365972.exambyte.domain.Test> tests = List.of(test1, test2);
        when(testRepository.findAll()).thenReturn(tests);

        // Act & Assert
        MvcResult result = mvc.perform(get("/organisator/tests")
                .with(oauth2Login().authorities(() -> "ROLE_ORGANIZER")))
                .andExpect(status().isOk())
                .andExpect(view().name("organisator/list"))
                .andExpect(model().attributeExists("tests"))
                .andReturn();

        @SuppressWarnings("unchecked")
        List<Test> modelTests = (List<Test>) result.getModelAndView().getModel().get("tests");
        assertThat(modelTests).hasSize(2)
                .extracting("titel")
    ...
    void test_createNewTest() throws Exception {
        // Arrange
        ArgumentCaptor<de.paul1365972.exambyte.domain.Test> testCaptor = ArgumentCaptor.forClass(de.paul1365972.exambyte.domain.Test.class);
        de.paul1365972.exambyte.domain.Test savedTest = new de.paul1365972.exambyte.domain.Test("New Test", LocalDateTime.now(), LocalDateTime.now(), LocalDateTime.now());
        savedTest.setId(1L);
        when(testRepository.save(any(de.paul1365972.exambyte.domain.Test.class))).thenReturn(savedTest);

        // Act
        mvc.perform(get("/organisator/tests/new")
                .with(oauth2Login().authorities(() -> "ROLE_ORGANIZER")))
    ...
    void test_editTestForm() throws Exception {
        // Arrange
        LocalDateTime now = LocalDateTime.now();
        de.paul1365972.exambyte.domain.Test test = new de.paul1365972.exambyte.domain.Test("Test 1", now, now.plusHours(2), now.plusHours(3));
        test.setId(1L);
        ...
        List<FrageMultipleChoiceItem> mcItems = List.of(
            new FrageMultipleChoiceItem("4", 1L, true),
        ...
        // Act & Assert
        MvcResult result = mvc.perform(get("/organisator/tests/1/edit")
                .with(oauth2Login().authorities(() -> "ROLE_ORGANIZER")))
                .andExpect(status().isOk())
                .andExpect(view().name("organisator/edit"))
                .andExpect(model().attributeExists("test", "questions", "mcItems"))
                .andReturn();

        de.paul1365972.exambyte.domain.Test modelTest = (de.paul1365972.exambyte.domain.Test) result.getModelAndView().getModel().get("test");
        ...
        @SuppressWarnings("unchecked")
        List<List<FrageMultipleChoiceItem>> modelMcItems = (List<List<FrageMultipleChoiceItem>>) result.getModelAndView().getModel().get("mcItems");
    ...
    void test_updateTest() throws Exception {
        ...
        // Act
        mvc.perform(post("/organisator/tests/1/edit")
                .with(oauth2Login().authorities(() -> "ROLE_ORGANIZER"))
                .with(csrf())
                .param("id", "1")
                .param("titel", "Updated Test")
                .param("startZeitpunkt", now.toString())
    ...
    void test_addQuestion() throws Exception {
        ...
        // Act
        mvc.perform(post("/organisator/tests/1/questions/new")
                .with(oauth2Login().authorities(() -> "ROLE_ORGANIZER"))
                .with(csrf())
                .param("typ", "MultipleChoice")
                .param("fragestellung", "Was ist 2+2?")
                .param("maximalePunktzahl", "2"))
                .andExpect(status().is3xxRedirection())
    ...
    void test_accessControl() throws Exception {
        // TODO: Aufteilen
        // Student test
        mvc.perform(get("/organisator/tests")
                .with(oauth2Login().authorities(() -> "ROLE_STUDENT")))
                .andExpect(status().isForbidden());

        // Korrektor test
        mvc.perform(get("/organisator/tests")
                .with(oauth2Login().authorities(() -> "ROLE_KORREKTOR")))
                .andExpect(status().isForbidden());
